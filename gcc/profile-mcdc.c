...
decision_tree_t *build_decision_tree(tree pred, int *counter)
{
   ...
   default:
	   ...
	   print_generic_stmt(stderr, pred, TDF_NONE);
	   break;
   }
   return res;
}
...
void create_mcdc_expression(tree old_cond_node, tree new_cond_node, int counter)
{
	...
	|| (TREE_CODE (old_cond_node) == TRUTH_ORIF_EXPR))
	{
		...
		dump_generic_node(&buffer, old_cond_node, 0, TDF_NONE, 0);
		...
		dump_generic_node(&buffer, new_cond_node, 0, TDF_NONE, 0);
		pp_newline (&buffer);
...
{
	return  build_call_nary(void_type_node, build_addr(tree_mcdc_profiler_fn),2, build_int_cst(integer_type_node,0), build_int_cst(integer_type_node, profile_mcdc_counter_id++));
}
#else
...
gimple_gen_mcdc_profiler (gimple_stmt_iterator gsi, int case_id)
{
  ...
  gcov_type_tmp_var = make_temp_ssa_name (gcov_type_node,
					  NULL, "PROF_mcdc_counter");
  stmt2 = gimple_build_assign(gcov_type_tmp_var, PLUS_EXPR, one); // expr: PROF_edge_counter + 1
